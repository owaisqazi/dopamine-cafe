...
import { useGetMenuByCategoryQuery, useGetByProductQuery } from "@/store/api/authApi";
...
export default function HomeMenu() {
  ...
  const [activeCategory, setActiveCategory] = useState<{ id: string; name: string }>({
    id: "",
    name: "All",
  });
  const activeCategoryId = activeCategory?.id || undefined;
  const { data: productsData, isLoading: productsLoading } = useGetByProductQuery(activeCategoryId, {
    skip: false,
    refetchOnMountOrArgChange: true,
  });
  const products: MenuItem[] = productsData?.data || [];
              ...
              >
                ...
                <div className="p-5">
                  <div className="flex justify-between items-start mb-2">
                    <h2 itemProp="name" className="text-lg font-bold text-gray-800">
                      {item?.name}
                    </h2>
                  ...
                  <p itemProp="description" className="text-sm text-gray-500 mb-4 line-clamp-2">
                    {item?.description}
                  </p>
